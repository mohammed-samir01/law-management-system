...
<div class="px-4 pt-4">
    ...
    <div class="mb-4">
        ...
        <h2 class="text-xl font-bold text-[#1E3A5F] dark:text-white">{{ auth()->user()->name }}</h2>
    </div>
    ...
    <div class="grid grid-cols-2 gap-3 mb-5">
        ...
        <div class="bg-[#1E3A5F] rounded-2xl p-4 text-white">
            <p class="text-3xl font-bold">{{ $stats['cases']['active'] }}</p>
            <p class="text-xs text-white/70 mt-1">قضايا نشطة</p>
        </div>

        <div class="bg-[#C9A84C] rounded-2xl p-4 text-white">
            <p class="text-3xl font-bold">{{ $stats['hearings']['today'] }}</p>
            <p class="text-xs text-white/70 mt-1">جلسات اليوم</p>
        </div>

        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <p class="text-3xl font-bold text-[#1E3A5F] dark:text-blue-300">{{ $stats['cases']['new'] }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">قضايا جديدة</p>
        </div>

        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <p class="text-3xl font-bold text-[#1E3A5F] dark:text-blue-300">{{ $stats['clients']['total'] }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">إجمالي العملاء</p>
        </div>

        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <p class="text-3xl font-bold text-orange-500">{{ $stats['invoices']['unpaid'] }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">فواتير غير مسددة</p>
        </div>

        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700">
            <p class="text-3xl font-bold text-[#1E3A5F] dark:text-blue-300">{{ $stats['hearings']['upcoming'] }}</p>
    ...
    <div class="mb-5">
        <div class="flex items-center justify-between mb-3">
            <h3 class="font-bold text-[#1E3A5F] dark:text-white">الجلسات القادمة</h3>
            <a href="{{ route('mobile.lawyer.hearings') }}" class="text-[#C9A84C] text-sm">عرض الكل</a>
        </div>
        <div class="space-y-2">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl p-3.5 shadow-sm border border-gray-100 dark:border-gray-700 flex items-center gap-3">
                <div class="w-12 h-12 bg-[#1E3A5F]/10 dark:bg-[#1E3A5F]/30 rounded-xl flex flex-col items-center justify-center shrink-0">
                    ...
                    <span class="text-[#1E3A5F] dark:text-blue-300 text-xs">{{ $hearing->scheduled_at->translatedFormat('M') }}</span>
                </div>
