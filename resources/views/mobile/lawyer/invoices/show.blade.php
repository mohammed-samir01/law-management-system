...
@section('title', 'تفاصيل الفاتورة — ميزان')

@section('header')
@section('header-title', 'تفاصيل الفاتورة')
@section('header-right')
<a href="{{ route('mobile.lawyer.invoices') }}" class="text-white/80 active:text-white">
...
<div class="px-4 pt-4">
    ...
    <div class="bg-[#1E3A5F] rounded-2xl p-5 text-white mb-4">
        <div class="flex items-start justify-between gap-2 mb-3">
            <div>
                ...
                @if($invoice->client)
                <p class="text-white/70 text-sm mt-1">{{ $invoice->client->getTranslation('name', 'ar', false) }}</p>
                @endif
            </div>
            <span @class([
                'text-xs px-3 py-1.5 rounded-full font-medium shrink-0',
                'bg-gray-400/30 text-gray-200'   => $invoice->status === 'draft',
                ...
                'bg-white/20 text-white/60'      => $invoice->status === 'cancelled',
    ...
    {{-- Notes --}}
    @if($invoice->getTranslation('notes', 'ar', false))
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 mb-4">
        <p class="text-xs text-gray-400 dark:text-gray-500 mb-1">ملاحظات</p>
        <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{{ $invoice->getTranslation('notes', 'ar', false) }}</p>
    </div>
    @endif
