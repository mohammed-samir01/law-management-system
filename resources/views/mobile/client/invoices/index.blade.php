...
@section('title', 'فواتيري — ميزان')

@section('header')
@section('header-title', 'فواتيري')
...
<div class="px-4 pt-4">
    ...
    <div class="flex gap-2 overflow-x-auto pb-1 mb-4 no-scrollbar">
        @foreach([''=>'الكل','sent'=>'غير مسددة','paid'=>'مدفوعة','overdue'=>'متأخرة'] as $val => $label)
        <a href="{{ route('mobile.client.invoices', ['status' => $val]) }}"
           ...
           class="block bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 active:opacity-70">
            <div class="flex items-start justify-between gap-2 mb-3">
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ $invoice->legalCase->getTranslation('title','ar',false) }}</p>
                    @endif
                </div>
                <span @class([
                    ...
                    'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400' => in_array($invoice->status, ['draft','cancelled']),
            ...
            <div class="flex items-center justify-between">
                ...
                @if($invoice->due_date)
                <p class="text-xs text-gray-400 dark:text-gray-500">استحقاق: {{ $invoice->due_date->format('d/m/Y') }}</p>
                @endif
            </div>
        </a>
        @empty
